...
class RedAlert():
    ...
    def get_locations_list(self):
        ...
        config_file_path = os.path.join(config_dir_path, 'targets.json')
    ...
    while True:
        ...
        if(red_alerts != None):
            ...
            for alert_city in red_alerts["data"]:
                ...
                '''
                logging.warning(f"city: {city_data}, time to run: {migun_time} seconds")
        else:
            logging.info("[-] No alerts for now, keep checking ...")

if __name__ == "__main__":
    log_file_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'red_alert.log')
